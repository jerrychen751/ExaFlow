...
def format_field_csv(velocity: np.ndarray, pressure: np.ndarray, origin: tuple[int, ...]) -> str:
    ...
    The column order is the axis indices, then the velocity components in axis order, then pressure. Rows run with the last axis varying fastest. Every value is written at full float64 precision, so the text reads back as the array that produced it.
    """
